...
# ── query routing ─────────────────────────────────────────────────────────────
CONV_EXACT = {
    "hello","hi","hey","hiya","howdy","yo","sup","heya",
    "how are you","how are you?","how r u","how r u?",
    "thanks","thank you","ty","thx","cheers","thank u",
    "okay","ok","got it","sure","alright","cool","noted",
    "bye","goodbye","see you","cya","later","take care",
    "interesting","nice","wow","great","awesome","amazing",
    "what can you do","what can you do?",
    "what all can you do","what all can you do?",
    ...
    "what can i ask here","what can i ask here?",
    "help","help me","how does this work","how does this work?",
    "what is insightx","what is insightx?",
    "who made you","who made you?","who built you","who created you",
    "are you an ai","are you ai","are you a bot",
    "lol","haha","hehe","ok cool","ok thanks","perfect",
    "i see","understood","makes sense","got it thanks",
    "no","yes","yep","nope","nah","yeah","yup",
    "good morning","good afternoon","good evening","good night",
}

# Regex patterns that always indicate conversational (non-data) questions
CONV_PATTERNS = [
    r"^what (all )?can you\b",
    r"^who (are|r) you\b",
    r"^what are you\b",
    r"^tell me (about yourself|more about you)\b",
    r"^(introduce yourself|about you)\b",
    r"^why don.t you\b",
    r"^why (aren.t|can.t|won.t|didn.t) you\b",
    r"^are you (an? )?(ai|bot|human|real|smart|good)\b",
    r"^do you (know|understand|remember|learn|think|feel)\b",
    r"^can you (help|explain|tell|show|talk|chat|understand|speak)\b",
    r"^(why|what do you think|tell me more|what does that mean)\b",
    r"^how (do|does|did) (you|this|insightx)\b",
    r"^(is this|is insightx|what is this)\b",
]
...
def needs_data(q):
    ...
    # Explicit conversational matches always win
    if ql in CONV_EXACT: return False
    for pat in CONV_PATTERNS:
        if re.match(pat, ql): return False
    # Only go to data pipeline if there are actual data keywords
    # Everything else is treated as conversational — no word-count heuristic
    return any(k in ql for k in DATA_KW)
...
def run_narrator(user_query, data_result, recommendations, tab="Default", tbl=None):
    """
    Narrator with full personality, conversation memory, and proper conversational handling.
    Analytical queries use pandas-verified data. Conversational queries use warm LLM personality.
    """
    llm  = get_llm()
    cat  = detect_query_category(user_query.lower())
    rec_text = "\n".join(f"- {r}" for r in recommendations) if recommendations else ""

    # Pandas-based fallback (always accurate for data queries)
    pandas_answer = _build_analytical_prose(user_query, data_result, tbl, recommendations, cat)

    # ── Static capability description used as last-resort for conversational ───
    INSIGHTX_PERSONA = (
        "I'm InsightX — your AI fintech analyst with 250,000 UPI transactions loaded. "
        "I can help you with fraud & risk analysis, bank failure rates, merchant category breakdowns, "
        "temporal trends (peak hours, day-of-week), age group analysis, device/network comparisons, "
        "custom charts, and full reports. Just ask in plain English — try something like "
        "'What are the top fraud-flagged banks?' or 'Show me peak transaction hours as a chart'."
    )

    if llm is None:
        if not data_result:
            return INSIGHTX_PERSONA
        return pandas_answer or "No data available."

    is_conversational = not bool(data_result)

    # ── Build the prompt ───────────────────────────────────────────────────────
    if is_conversational:
        # Get recent history for context-aware responses
        history = get_history(tab)
        recent = history[-(MAX_HISTORY_TURNS * 2):]
        context_note = ""
        if recent:
            context_note = "Prior context in this conversation (use naturally, don't repeat):\n"
            for msg in recent[-6:]:
                role = "User" if msg["role"] == "user" else "You"
                context_note += f"{role}: {msg['content'][:120]}\n"
            context_note += "\n"

        sys_msg = (
            "You are InsightX — a sharp, witty, and genuinely helpful AI fintech analyst. "
            "You have a real personality: confident, direct, occasionally funny, and always honest.\n\n"
            "You have 250,000 Indian UPI transactions loaded covering fraud flags, bank performance, "
            "merchant categories, device/network usage, age groups, and temporal patterns.\n\n"
            "IMPORTANT RULES:\n"
            "1. NEVER give a data dump or stats table for a conversational question — that's a different mode.\n"
            "2. Talk like a person, not a brochure. Vary your sentence structure.\n"
            "3. If asked what you can do — be specific and enthusiastic. Give real example questions.\n"
            "4. If asked something dumb or random — own it, be playful, redirect with warmth.\n"
            "5. Reference previous messages in the conversation naturally when relevant.\n"
            "6. End analytical chat with a follow-up question that could lead somewhere interesting.\n"
            "7. NEVER say 'I cannot', 'I don't have access', or 'as an AI'. You're InsightX — act like it.\n"
            "8. Complete every sentence. Never trail off."
        )
        user_turn = (
            f"{context_note}"
            f"Current message: \"{user_query}\"\n\n"
            "Respond naturally in 3-6 sentences. Match the energy of the message — "
            "casual if they're casual, focused if they're asking something specific. "
            "If it's about capabilities, list 4 real example questions with context. "
            "If it's a greeting, be warm and invite them in. "
            "If it's weird or silly, roll with it and bring it back to what you can help with."
        )
        temperature = 0.6
        max_tokens  = 600
    else:
        sys_msg = (
            "You are InsightX, a precise and confident fintech data analyst.\n"
            "Rules:\n"
            "1. Start with the direct answer and exact number from the data.\n"
            "2. Name EVERY category with its exact value — do NOT omit any.\n"
            "3. Explain the underlying pattern in plain language.\n"
            "4. End with one specific actionable recommendation.\n"
            "5. Write flowing prose — no bullet points, no headers.\n"
            "6. Use ONLY numbers from the Computed Data section. Never invent figures.\n"
            "7. For fraud_flag: always say 'flagged for review, not confirmed fraud'.\n"
            "8. Write COMPLETE sentences. Never stop mid-thought.\n"
            "9. Be direct — avoid wishy-washy hedging phrases."
        )
        user_turn = (
            f"[{cat.upper()} QUERY] User asked: \"{user_query}\"\n\n"
            f"COMPUTED DATA (use these exact numbers):\n{data_result}\n\n"
            f"RECOMMENDATIONS:\n{rec_text}\n\n"
            "Write a COMPLETE analytical response: (1) direct answer + top finding with exact value, "
            "(2) list ALL categories with individual values, "
    ...
    def _run_llm(prompt_text, temp):
        try:
            out = llm(prompt_text, max_tokens=max_tokens, stop=STOP_TOKENS,
                      echo=False, temperature=temp, repeat_penalty=1.1)
            raw = out["choices"][0]["text"]
    ...
    def _score_llm(text, expected_tbl):
        ...
        hallucination_markers = [
            ...
            "x%","y%","z%","figure x","figure y","value x","value a",
        ]
        ...
        if expected_tbl:
            ...
            score += int((names_found / max(len(rows), 1)) * 15)
        return score

    # ── Conversational: single LLM call, no data-quality gates ────────────────
    if is_conversational:
        llm_reply = _run_llm(prompt, temperature)
        if llm_reply and len(llm_reply.split()) >= 5:
            return llm_reply
        return INSIGHTX_PERSONA

    # ── Analytical: up to 3 attempts, keep best, fall back to pandas ──────────
    best_llm   = None
    best_score = -1
    for _ in range(3):
        candidate = _run_llm(prompt, temperature)
        if candidate:
    ...
    # Only flag as vague for data queries (not conversational replies)
    is_vague = any(phrase in llm_answer.lower() for phrase in [
        "i cannot","i don't have","not available","based on the data provided",
        "the data shows","considering the",
        "certainly, here","certainly here","sure, here",
    ...
    return "I couldn't find relevant data for that query. Try asking about fraud rates, bank failures, merchant revenue, or peak transaction hours."
...
def process_query(user_query, tab="Default"):
    ...
    # ── CONVERSATIONAL FAST PATH — bypass all data pipeline entirely ──────────
    # This must come before _direct_analytics which always generates a nav string
    is_conv = not needs_data(user_query) and not wants_chart(user_query) \
              and not wants_table(user_query) and not wants_report(user_query) \
              and sum_mode is None and not wants_stats_table(user_query)

    if is_conv:
        text = run_narrator(user_query, "", [], tab, tbl=None)
        result["text"] = text
        _push(user_query, text)
        return result

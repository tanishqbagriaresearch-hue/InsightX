...
def _direct_analytics(q_lower, df):
    ...
    # ── COMPARATIVE ──────────────────────────────────────────────────────────
        ...
        if _asking_tx_fail or _generic_fail_by_type:
        ...
        elif ("android" in q_lower or "ios" in q_lower or "device" in q_lower):
        ...
        elif "network" in q_lower or "4g" in q_lower or "5g" in q_lower or "wifi" in q_lower:
            rows = []
    ...
    # ── SEGMENTATION ─────────────────────────────────────────────────────────
        ...
        if "p2p" in q_lower and ("age" in q_lower or "group" in q_lower or "who" in q_lower):
            sub = df[df["transaction_type"]=="P2P"]
            order = ["18-25","26-35","36-45","46-55","56+"]
            ...
            g = df.groupby("sender_bank").agg(
                count=("transaction_type","count"),
                avg_amt=(_amt(df) or "amount_inr","mean"),
                fail_rate=("transaction_status",lambda x:(x=="FAILED").mean()*100),
                flag_rate=("fraud_flag","mean")
            ).sort_values("count",ascending=False) if _amt(df) else None
            if g is not None:
    ...
    # ── CORRELATION ───────────────────────────────────────────────────────────
        ...
        if ("network" in q_lower and "success" in q_lower) or \
           ("network" in q_lower and "fail" in q_lower) or \
           ("network" in q_lower and "transaction" in q_lower):
            g = df.groupby("network_type").agg(
                count=("transaction_status","count"),
                success_rate=("transaction_status",lambda x:(x=="SUCCESS").mean()*100),
                fail_rate=("transaction_status",lambda x:(x=="FAILED").mean()*100),
                flag_rate=("fraud_flag","mean"),
            ...
            rows = [[net,f"{int(row['count']):,}",
                     f"{row['success_rate']:.2f}%",f"{row['fail_rate']:.2f}%",
                     f"{row['flag_rate']*100:.2f}%",f"Rs.{row['avg_amt']:,.2f}"]
                    for net,row in g.iterrows()]
            tbl = {"headers":["Network","Count","Success%","Fail%","Flagged%","Avg Amount"],"rows":rows}
        ...
        elif "device" in q_lower and ("success" in q_lower or "fail" in q_lower or "transaction" in q_lower):
            g = df.groupby("device_type").agg(
                count=("transaction_status","count"),
                success_rate=("transaction_status",lambda x:(x=="SUCCESS").mean()*100),
                fail_rate=("transaction_status",lambda x:(x=="FAILED").mean()*100),
                flag_rate=("fraud_flag","mean"),
            ).sort_values("fail_rate",ascending=False)
            rows = [[dev,f"{int(row['count']):,}",f"{row['success_rate']:.2f}%",
                     f"{row['fail_rate']:.2f}%",f"{row['flag_rate']*100:.2f}%"]
                    for dev,row in g.iterrows()]
            tbl = {"headers":["Device","Count","Success%","Fail%","Flagged%"],"rows":rows}
            ...
            # Generic: network × transaction_type crosstab
            ct = pd.crosstab(df["network_type"],df["transaction_type"],
                             values=df["transaction_status"].apply(lambda x: 1 if x=="SUCCESS" else 0),
                             aggfunc="mean")*100
            rows = [[net]+[f"{ct.loc[net,t]:.1f}%" if t in ct.columns else "N/A"
                           for t in df["transaction_type"].unique()]
                    for net in ct.index]
            tbl = {"headers":["Network"]+list(df["transaction_type"].unique()),"rows":rows}
